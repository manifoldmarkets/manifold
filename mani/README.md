...
- Sentry problems? `sentry.properties` file not found or malformed?
  ...
  ### Generating new android signing keys
...
- `android/appdebug.keystore` is generated automatically, but when copied from native, I decided to recreate to be sure it would work:
  ...
  ```
  keytool -genkey -v -keystore debug.keystore -alias androiddebugkey -storepass android -keypass android -keyalg RSA -validity 10000 -keysize 2048
  ```
...
- and filled the info in `credentials.json`.
- I got the SHA fingerprint via:

  ```
  keytool -list -v -keystore android/app/debug.keystore -alias androiddebugkey -storepass android | grep SHA1
  ```

- then added this to the mani android app in the firebase project settings. I downloaded the google-services.json file and added it to the `configs/{env}` directory. I got the client id from the google-services.json file after adding the sha fingerprint to the app and added it to the `dev.ts` and `prod.ts` files. I also enabled custom uri schemes in the google console app's oauth client id screen.
