...
export function LoadingOverlay(props: { visible: boolean; message: string; loading: boolean, className?: string, spinnerBorderColor?: string }) {
    ...
    return (
        true && (
            <div className={clsx("absolute inset-0 flex justify-center items-center bg-gray-500 bg-opacity-75 z-[9999] text-white transition-opacity duration-500", !visible && "opacity-0 pointer-events-none", className)}>
