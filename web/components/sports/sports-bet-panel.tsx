...
export function SportsBetPanel({
  ...
  const [selectedExpiration, setSelectedExpiration] = usePersistentLocalState<number>(
    0,
  ...
  const expiresMillisAfter =
    selectedExpiration > 0 && selectedExpiration !== -1 ? selectedExpiration : undefined
  ...
  async function handleBet() {
    if (!user) { firebaseLogin(); return }
    if (!match.contractId || !current.answerId) {
      if (match.marketUrl) window.open(match.marketUrl, '_blank')
      return
    }
    if (!betAmount || betAmount < 1) { toast.error('Enter a bet amount'); return }
    if (betMode === 'limit' && (!limitProbInt || limitProbInt < 1 || limitProbInt > 99)) {
    ...
    <Modal open setOpen={(open) => { if (!open) onClose() }} size="md">
      <Col className="w-full overflow-hidden rounded-xl bg-canvas-0 text-ink-1000">
        ...
        <Col className="gap-5 px-6 pb-6 pt-5">
                ...
                <button
                  ...
                  className={clsx(
                    ...
                    betMode === mode
                      ...
                      : 'text-ink-400 font-medium hover:text-ink-600'
                  )}
                >
              ...
              <Col className="gap-1">
                ...
                <label className="relative w-full">
                  <Input
                    ...
                    value={limitProbInt ?? ''}
                    onChange={(e) =>
                      setLimitProbClamped(e.target.value === '' ? undefined : Number(e.target.value))
                    }
                  />
                  <Row className="absolute right-2 top-3.5 gap-1.5">
                    {([-5, -1, 1, 5] as const).map((delta) => (
                      <button
                        ...
                        onClick={() => setLimitProbClamped((limitProbInt ?? current.prob) + delta)}
                      >
                        {delta > 0 ? `+${delta}` : delta}
                      </button>
                    ))}
          ...
          <Row className="items-center justify-between">
            <span className="text-ink-400 text-sm">
              {user ? `Your mana balance ${formatMoney(user.balance)}` : 'Sign in to place bets'}
            </span>
            {match.marketUrl && (
              <a
                href={match.marketUrl}
