...
export function usePersistentApiPolling<T extends TableName>(
  table: T,
  allRowsQ: Promise<Row<T>[]>,
  onlyNewRowsQ: (results: Row<T>[] | undefined) => Promise<Row<T>[]>,
  key: string,
  opts?: PollingOptions
) {
  const { ms, deps, shouldUseLocalStorage } = opts ?? {
  ...
  const runOnlyNewRowsQ = useEvent(async () => {
    return await onlyNewRowsQ(results)
  })
  const runAllRowsQ = useEvent(async () => {
    return await allRowsQ
  })
  const updateResults = useEvent((rows: Row<T>[]) => {
    setResults(insertChanges(table, results ?? [], rows))
  })
