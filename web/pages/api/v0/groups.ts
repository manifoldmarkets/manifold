...
export default async function handler(
  ...
  const beforeTime = params.beforeTime ? parseInt(params.beforeTime) : undefined
  ...
  if (availableToUserId) {
    ...
    const publicGroups = await getPublicGroups(500, beforeTime)
  ...
  const groups = await getPublicGroups(500, beforeTime)
  res.setHeader('Cache-Control', 'max-age=60')
  res.status(200).json(groups)
}
