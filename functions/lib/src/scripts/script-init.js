...
exports.initAdmin = exports.getServiceAccountCredentials = void 0;
...
const getFirebaseProjectRoot = (cwd) => {
...
const getFirebaseActiveProject = (cwd) => {
...
const getServiceAccountCredentials = (env) => {
    env = env || getFirebaseActiveProject(process.cwd());
    if (env == null) {
        throw new Error("Couldn't find active Firebase project; did you do `firebase use <alias>?`");
    }
    const envVar = `GOOGLE_APPLICATION_CREDENTIALS_${env.toUpperCase()}`;
    const keyPath = process.env[envVar];
    if (keyPath == null) {
        throw new Error(`Please set the ${envVar} environment variable to contain the path to your ${env} environment key file.`);
    }
...
const initAdmin = (env) => {
    try {
        const serviceAccount = (0, exports.getServiceAccountCredentials)(env);
...
//# sourceMappingURL=script-init.js.map