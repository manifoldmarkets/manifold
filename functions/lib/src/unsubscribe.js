...
exports.unsubscribe = {
    ...
    handler: async (req, res) => {
        ...
        const notificationSubscriptionType = type;
        if (notificationSubscriptionType === undefined) {
            res.status(400).send('Invalid subscription type parameter.');
            return;
        }
        const optOutAllType = 'opt_out_all';
        const wantsToOptOutAll = notificationSubscriptionType === optOutAllType;
        const user = await (0, utils_1.getPrivateUser)(id);
        if (!user) {
            res.send('This user is not currently subscribed or does not exist.');
            return;
        }
        const previousDestinations = user.notificationPreferences[notificationSubscriptionType];
        let newDestinations = previousDestinations;
        if (wantsToOptOutAll)
            newDestinations.push('email');
        else
            newDestinations = previousDestinations.filter((destination) => destination !== 'email');
        console.log(previousDestinations);
        const { email } = user;
        const update = {
            notificationPreferences: Object.assign(Object.assign({}, user.notificationPreferences), { [notificationSubscriptionType]: newDestinations }),
        };
                  ...
                  style="direction:ltr;font-size:0px;padding:0px 0px 0px 0px;padding-bottom:0px;padding-left:0px;padding-right:0px;padding-top:0px;text-align:center;">
                    ...
                    <div class="mj-column-per-100 mj-outlook-group-fix"
                      ...
                      ${email} has been unsubscribed from email notifications related to:
                                            ...
                                            <span style="font-weight: bold; color:#000000;font-family:Arial, Helvetica, sans-serif;font-size:18px;">${notification_1.NOTIFICATION_DESCRIPTIONS[notificationSubscriptionType].detailed}.</span>
                                        </p>
                                        <br/>
                                        <br/>
                                        <br/>
