...
async function correctContractCreatorInfo() {
    ...
    await Promise.all(users.map(async (user) => {
        const contracts = await (0, utils_1.getValues)(firestore.collection('contracts').where('creatorId', '==', user.id));
        await Promise.all(contracts.map(async (contract) => {
            if (contract.creatorUsername !== user.username ||
        ...
        const betsSnap = await firestore
            ...
            .get();
        await Promise.all(betsSnap.docs.map(async (doc) => {
            const bet = doc.data();
            if ((bet.userUsername !== user.username ||
                bet.userName !== user.name ||
                bet.userAvatarUrl !== user.avatarUrl) &&
                bet.createdTime > 1654041600000) {
                console.log(`Updating bet ${doc.id} on contract ${bet.contractId} creator info`);
        ...
        const answerSnap = await firestore
            ...
            .where('userId', '==', user.id)
            .get();
        await Promise.all(answerSnap.docs.map(async (doc) => {
            const answer = doc.data();
            if (answer.username !== user.username ||
                answer.name !== user.name ||
                answer.avatarUrl !== user.avatarUrl) {
                console.log(`Updating answer ${doc.id} on contract ${answer.contractId} creator info`);
                await doc.ref.update({
        ...
        const commentSnap = await firestore
            ...
            .where('userId', '==', user.id)
            .get();
        await Promise.all(commentSnap.docs.map(async (doc) => {
            const comment = doc.data();
            if (comment.userUsername !== user.username ||
...
//# sourceMappingURL=correct-contract-creator-info.js.map