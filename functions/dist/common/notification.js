...
exports.ReactionNotificationTypes = exports.getSourceUrl = exports.getSourceIdForLinkComponent = exports.NOTIFICATION_DESCRIPTIONS = void 0;
const group_1 = require("./group");
const user_1 = require("./user");
exports.NOTIFICATION_DESCRIPTIONS = {
    ...
    comments_by_followed_users_on_watched_markets: {
        ...
        detailed: 'Only comments by users that you follow on markets that you watch',
    },
    ...
    loan_income: {
        ...
        detailed: 'Automatic loans from your predictions that are locked in unresolved markets',
    },
    ...
    market_updates_on_watched_markets_with_shares_in: {
        simple: `Only creator updates on markets that you've ${user_1.PAST_BET}`,
        detailed: `Only updates made by the creator on markets that you've ${user_1.PAST_BET}`,
    },
    on_new_follow: {
    ...
    resolutions_on_watched_markets_with_shares_in: {
        simple: `Only market resolutions that you've ${user_1.PAST_BET}`,
        detailed: `Only resolutions of markets you're watching and that you've ${user_1.PAST_BET}`,
    },
    ...
    all_comments_on_contracts_with_shares_in_on_watched_markets: {
        simple: `Only on markets you've ${user_1.PAST_BET}`,
        detailed: `Comments on markets that you're watching and you've ${user_1.PAST_BET}`,
    },
    ...
    all_answers_on_contracts_with_shares_in_on_watched_markets: {
        simple: `Only on markets you've ${user_1.PAST_BET}`,
        detailed: `Answers on markets that you're watching and that you've ${user_1.PAST_BET}`,
    },
    opt_out_all: {
        simple: 'Opt out of all notifications (excludes when your markets close)',
        detailed: 'Opt out of all notifications excluding your own market closure notifications',
    },
...
function getSourceIdForLinkComponent(sourceId, sourceType) {
    switch (sourceType) {
        case 'answer':
...
exports.getSourceIdForLinkComponent = getSourceIdForLinkComponent;
function getSourceUrl(notification) {
    const { sourceType, sourceId, sourceUserUsername, sourceContractCreatorUsername, sourceContractSlug, sourceSlug, } = notification;
    if (sourceType === 'follow')
        return `/${sourceUserUsername}`;
    if (sourceType === 'group' && sourceSlug)
        return `${(0, group_1.groupPath)(sourceSlug)}`;
    ...
    if (sourceType === 'challenge' ||
        exports.ReactionNotificationTypes.includes(sourceType))
        return `${sourceSlug}`;
    if (sourceContractCreatorUsername && sourceContractSlug)
        return `/${sourceContractCreatorUsername}/${sourceContractSlug}#${getSourceIdForLinkComponent(sourceId !== null && sourceId !== void 0 ? sourceId : '', sourceType)}`;
    else if (sourceSlug)
        return `${sourceSlug.startsWith('/') ? sourceSlug : '/' + sourceSlug}#${getSourceIdForLinkComponent(sourceId !== null && sourceId !== void 0 ? sourceId : '', sourceType)}`;
    return '';
}
exports.getSourceUrl = getSourceUrl;
exports.ReactionNotificationTypes = [
    'comment_like',
    'contract_like',
];
