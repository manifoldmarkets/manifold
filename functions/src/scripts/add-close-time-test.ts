// Some markets don't have a close time. Displays the close times that would be added by add-close-time.ts.
...
async function addCloseTimeToContract(contract: Contract) {
  ...
  console.log('Defined close time', contract.slug, new Date(closeTime))
}
