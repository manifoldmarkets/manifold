...
alter table love_answers add constraint love_answers_question_creator_unique unique (question_id, creator_id);
...
create policy  "self update" on love_answers for update
    with check (creator_id = firebase_uid());
