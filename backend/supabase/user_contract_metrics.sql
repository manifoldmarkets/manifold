...
create unique index unique_user_contract_answer on user_contract_metrics (user_id, contract_id, coalesce(answer_id, ''));
...
create index contract_metrics_no_shares on user_contract_metrics (contract_id, total_shares_no desc)
where
  total_shares_no is not null;

create index contract_metrics_yes_shares on user_contract_metrics (contract_id, total_shares_yes desc)
where
  total_shares_yes is not null;
...
CREATE INDEX idx_profit ON public.user_contract_metrics (profit);
