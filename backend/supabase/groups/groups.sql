create
or replace function get_contracts_in_group_slugs_1 (
  contract_ids text[],
  p_group_slugs text[],
  ignore_slugs text[]
) returns table (data JSON, importance_score numeric) stable parallel safe language sql as $$
...
create
or replace function get_recently_active_contracts_in_group_slugs_1 (
  p_group_slugs text[],
  ignore_slugs text[],
  max int
) returns table (data JSON, importance_score numeric) stable parallel safe language sql as $$
