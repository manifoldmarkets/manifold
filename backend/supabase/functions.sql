...
select
  data ->> 'contentId' as contract_id,
  count(*) as n
from
  user_reactions
where
  data ->> 'contentType' = 'contract'
...
from
  user_recommendation_features as urf
...
from
  ...
  cross join contract_recommendation_features as crf
where
  user_id = uid -- That has not been viewed.
  and not exists (
    select
      1
    from
...
create
or replace function get_recommended_contracts_by_score_excluding(
  uid text,
  count int,
...
or replace function get_recommended_contracts(uid text, n int, excluded_contract_ids text []) returns setof jsonb language plpgsql as $ $ begin create temp table your_recs on commit drop as (
  select
    *
  from
    get_recommended_contract_set(uid, n, excluded_contract_ids)
);
...
create
or replace function get_cpmm_pool_prob(pool jsonb, p numeric) returns numeric language plpgsql immutable parallel safe as $ $ declare p_no numeric := (pool ->> 'NO') :: numeric;
...
select
  millis_to_ts(end_millis) - millis_to_ts(start_millis) $ $;
...
from
  contract_recommendation_features as crf,
  target_contract
where
  crf.contract_id != target_contract.contract_id
order by
  distance $ $;
...
from
  (
    ...
    from
      contracts
    where
      data -> 'groupSlugs' ? | group_slugs
      and is_valid_contract(contracts)
    order by
      (data -> 'uniqueBettors7Days') :: int desc,
...
alter table
  discord_users enable row level security;
...
alter table
  discord_messages_markets enable row level security;
...
select
  contract_id
from
  your_liked_contracts
union
select
  contract_id
from
  your_followed_contracts $ $;
...
order by
  similarity * similarity * log(popularity_score + 100) desc
limit
  match_count;
