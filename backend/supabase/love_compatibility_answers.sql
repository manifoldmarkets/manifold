...
CREATE POLICY "self insert" ON love_compatibility_answers FOR INSERT WITH CHECK ((creator_id = firebase_uid()));

DROP POLICY IF EXISTS "self update" ON love_compatibility_answers;
CREATE POLICY "self update" ON love_compatibility_answers FOR UPDATE USING ((creator_id = firebase_uid())) ;
