...
create policy "self read" on private_user_message_channel_members using (user_id = firebase_uid ());

-- SECURITY
create
or replace function can_access_private_messages (channel_id bigint, user_id text) returns boolean parallel safe language sql as $$
select exists (
...
create policy "private read" on private_user_message_channel_members using (
...
create policy "private read" on private_user_messages using (
  firebase_uid () is not null
  and can_access_private_messages (channel_id, firebase_uid ())
);

create index if not exists private_user_messages_channel_id_idx on private_user_messages (channel_id, created_time desc);
...
create index if not exists user_seen_private_messages_created_time_desc_idx on private_user_seen_message_channels (user_id, channel_id, created_time desc);
