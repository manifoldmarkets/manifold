...
import { resolveQuestion } from 'api/resolve-question'
...
if (require.main === module) {
  ...
  resolveQuestionsAgain(contractIds).then(() => process.exit(0))
}

async function resolveQuestionsAgain(contractIds: string[]) {
  ...
  for (const [contract, creator, resolutionParams] of params) {
    if (contract && creator && resolutionParams) {
      ...
      try {
        await resolveQuestion(contract, creator, resolutionParams)
      } catch (e) {
        console.log(e)
      }
    }
