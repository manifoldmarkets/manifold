...
import { QuestionAdRedeemFeeTxn, QuestionAdRedeemTxn } from 'common/txn'
...
export const redeemboost = authEndpoint(async (req, auth) => {
  ...
  if (count > 1) {
    throw new APIError(
      403,
      `You have already redeemed the boost for this question ${count} times`
    )
  }
  ...
  if (funds < cost) {
    throw new APIError(
      403,
      'Ad for question does not have enough funds to pay out'
    )
  }
  ...
  const result = await firestore.runTransaction(async (trans) => {
    ...
    txnColl.add({
      ...
      amount: reward,
      token: 'M$',
      description: 'Redeeming question ad',
    ...
    txnColl.add({
      ...
      amount: cost - reward,
      token: 'M$',
      description: 'Manifold fee for redeeming question ad',
      createdTime: Date.now(),
    } as QuestionAdRedeemFeeTxn)
