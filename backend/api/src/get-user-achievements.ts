...
export const getUserAchievements: APIHandler<'get-user-achievements'> = async ({
  ...
  // Detect if the MV exists; if not, we'll skip rank/percentile columns.
  ...
  const queryWithMV = `with base as (
      ...
      select
        ...
        coalesce(league_ranks.seasons_rank1_by_cohort, 0) as seasons_rank1_by_cohort,
        coalesce(league_ranks.seasons_rank1_masters, 0) as seasons_rank1_masters,
  ...
  const result = await pg.oneOrNone(hasMV ? queryWithMV : queryWithoutMV, [
    userId,
  ])
