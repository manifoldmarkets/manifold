...
import { canUserAddGroupToQuestion } from './add-contract-to-group'
...
export const createquestion = authEndpoint(async (req, auth) => {
  return createQuestionHelper(req.body, auth)
})

export async function createQuestionHelper(body: schema, auth: AuthedUser) {
  const {
    question,
    description,
    descriptionHtml,
  ...
  const { user, contract } = await firestore.runTransaction(async (trans) => {
    ...
    if (user.isBannedFromPosting)
      throw new APIError(400, 'User banned from creating questions.')
...
function validateQuestionBody(body: any) {
  const {
    question,
    description,
    descriptionHtml,
  ...
  if (visibility == 'private' && !groupId) {
    throw new APIError(
      400,
      'Private questions cannot exist outside a private group.'
    )
  }
...
async function getGroup(groupId: string, visibility: string, userId: string) {
  ...
  if (
    ...
    throw new APIError(
      400,
      `Both "${group.name}" and question must be of the same private visibility.`
    )
  }

  if (
    !canUserAddGroupToQuestion({
      userId,
      group,
      membership,
    })
