...
> = async (body, auth) => {
  ...
  const winnerWord = winnerCount === 1 ? 'winner' : 'winners'
  const body_text = `Drawing #${sweepstakesNum}: $${totalPrizeUsd.toLocaleString()} across ${winnerCount} ${winnerWord}. Enter before the drawing closes.`
