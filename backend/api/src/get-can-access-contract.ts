...
export const getcanaccesscontract = authEndpoint(async (req, auth) => {
  ...
  const groupId = contract.groupLinks.groupId

  // checks if user is member
  const userCanAccess = await pg.one(
    `select exists(
        select * from group_role
        where group_id = '${groupId}'
        and member_id='${auth.uid}')`
  )
  return userCanAccess.exists
})
