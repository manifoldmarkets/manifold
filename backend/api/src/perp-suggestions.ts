...
const SELECT_SUGGESTION = `
  select s.id, s.user_id, s.name, s.data_source, s.created_time,
...
export const getPerpSuggestions: APIHandler<'get-perp-suggestions'> = async (
  ...
  return pg.map(
    `${SELECT_SUGGESTION}
     where not s.hidden
     order by votes desc, s.created_time desc
     limit $2`,
...
export const createPerpSuggestion: APIHandler<
...
> = async (props, auth) => {
  ...
  // way. The unique index makes the insert race-safe.
...
export const votePerpSuggestion: APIHandler<'vote-perp-suggestion'> = async (
  props,
  auth
) => {
