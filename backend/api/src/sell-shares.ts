...
export const sellShares = typedEndpoint(
  'sell-shares',
  async (props, auth, { log }) => {
    ...
    const result = await firestore.runTransaction(async (transaction) => {
      ...
      const [userBetsSnap, { unfilledBets, balanceByUserId }] =
        await Promise.all([
      ...
      transaction.update(userDoc, {
        balance: FieldValue.increment(
          -newBet.amount + (newBet.loanAmount ?? 0)
        ),
      })
    ...
    if (
      ...
      floatingEqual(maxShares, soldShares)
    ) {
