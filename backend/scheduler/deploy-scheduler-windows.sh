...
# Run from Git Bash: ./deploy-scheduler-windows.sh [dev|prod] [main|perps]
#
...
if [ "$TARGET" = "all" ]; then
    ...
    echo "  ./deploy-scheduler-windows.sh $ENV main    # batch jobs (instance: scheduler)"
    echo "  ./deploy-scheduler-windows.sh $ENV perps   # PERP oracle/funding jobs (instance: scheduler-perps)"
