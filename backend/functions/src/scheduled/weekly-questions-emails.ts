...
import { sendInterestingQuestionsEmail } from 'shared/emails'
...
export const weeklyQuestionsEmails = functions
  ...
  .timeZone('Etc/UTC')
  .onRun(async () => {
    await sendTrendingQuestionsEmailsToAllUsers()
  })

const firestore = admin.firestore()

export async function sendTrendingQuestionsEmailsToAllUsers(isDebug = false) {
  const privateUsers =
    !isProd() || isDebug
      ? filterDefined([
  ...
  const privateUsersToSendEmailsTo = privateUsers
    ...
    .filter(
      (user) =>
        user.notificationPreferences.trending_questions.includes('email') &&
...
const sendEmailToPrivateUser = async (
  ...
  await sendInterestingQuestionsEmail(user, privateUser, contractsToSend)
}
