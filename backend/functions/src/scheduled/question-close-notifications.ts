...
import { createQuestionClosedNotification } from 'shared/create-notification'
import { DAY_MS } from 'common/util/time'
import { secrets } from 'common/secrets'

const SEND_NOTIFICATIONS_EVERY_DAYS = 5
export const questionCloseNotifications = functions
  .runWith({ secrets, memory: '4GB', timeoutSeconds: 540 })
  .pubsub.schedule('every 1 hours')
  .onRun(async () => {
    isProd()
      ? await sendQuestionCloseEmails()
      : console.log('Not prod, not sending emails')
  })

const firestore = admin.firestore()

export async function sendQuestionCloseEmails() {
  ...
  for (const contract of contracts) {
    ...
    await createQuestionClosedNotification(
      contract,
      user,
      privateUser,
...
// day of a multiple of the time period after the question has closed, it won't
...
function shouldSendFirstOrFollowUpCloseNotification(contract: Contract) {
  ...
  const { closedMultipleOfNDaysAgo, fullTimePeriodsSinceClose } =
    questionClosedMultipleOfNDaysAgo(contract)
  return (
...
function questionClosedMultipleOfNDaysAgo(contract: Contract) {
  const now = Date.now()
