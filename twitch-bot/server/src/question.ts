...
export class Question {
  ...
  static async loadFromManifoldID(app: App, manifoldID: string, stream: TwitchStream) {
    const question = new Question(app, stream);
    try {
      const [contractDoc, betCollection] = await app.manifoldFirestore.getFullQuestionByID(manifoldID);
      let initializing = true;
      await new Promise<void>((resolve, reject) =>
        question.firestoreSubscriptions.push(
          onSnapshot(contractDoc, (update) => {
            try {
              ...
              question.data = {
                ...
                probability: contract.outcomeType === 'BINARY' ? Question.getProbability(binaryContract) : undefined,
                isResolved,
                resolutionTime,
                resolution,
                p,
        ...
        question.firestoreSubscriptions.push(
          onSnapshot(betCollection, async (update) => {
            if (initializing) {
              ...
              bets.filter((b) => Question.isBetValid(b));
              ...
              await Promise.all(bets.map(async (b) => <NamedBet>{ ...b, username: await app.getDisplayNameForUserID(b.userId) })).then((bets) => (question.data.bets = bets));
              r();
              return;
            }
            ...
            for (const changedBet of changes) {
              if (changedBet.type === 'added') {
                question.onNewBet(changedBet.doc.data());
              }
            }
          })
        )
  ...
  private async resolve() {
    ...
    this.stream.questionResolved(this);
  }
  ...
  static calculateFixedPayout(contract: AbstractQuestion, bet: Bet, outcome: string) {
  ...
  static calculateFixedMktPayout(contract: AbstractQuestion, bet: Bet) {
  ...
  static resolvedPayout(contract: AbstractQuestion, bet: Bet) {
  ...
  async calculateWinners(): Promise<{ user: LiteUser; profit: number }[]> {
    ...
    for (const bet of bets) {
      if (bet.sale) {
        ...
        profitById[bet.id] = Question.resolvedPayout(this.data, bet) - bet.amount;
      }
    }

    const openBets = bets.filter((bet) => !bet.isSold && !bet.sale);
    const betsByUser = groupBy(openBets, 'userId');
    const userProfits = mapValues(betsByUser, (bets) => sumBy(bets, (bet) => Question.resolvedPayout(this.data, bet) - bet.amount));
