...
| `sports-odds-create`  | daily 06:00 LA                       | For every competition with an auto-create phase inside the next 14 days: fetch the games in that window, skip the ones that already have a market, create the rest as @ManifoldSports. Also on demand, with a dry run, from `/admin/sports`.                                                                               |
...
| shape                                                                                                 | **Versus market**: multiple choice with two answers, home then away, summing to one, adding disabled; a tie resolves 50/50. Soccer (or `tiesAllowed` on the calendar entry): three answers, home, away, `Draw`. Every answer opens at the bookmakers' devigged moneyline (`answerProbs`); a game with no line yet waits for the next daily run. `buildOddsMarketParams` in `common/src/odds-markets.ts` decides. Markets created before the switch are binary (YES is the home team) and still resolve.                                                                                              | `toOfficialGame`, the bet dialogs                                      |
| answer colours                                                                                        | the teams' colours, home first, with a neutral grey Draw (`gameAnswerColors` in `common/src/sports-team-colors.ts`). Brand colours are fitted into a luminance band: darker for versus markets, whose side buttons carry white text, lighter for three-way markets, whose answer bars sit behind text. A matchup with no two colours that read apart (CIEDE2000 under 20), or a team with no colours on file (college football), keeps the defaults: the versus indigo and amber, or the chart palette. `backend/scripts/backfill-sports-answer-colors.ts` recolours open games created before this. | bet buttons, answer bars, the chart, the team badge on `/sports`       |
| `sportsHomeTeam`, `sportsAwayTeam`                                                                    | the team names from the provider                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | matching answers at resolution, prop matching by team name             |
...
| `sportsStartTimestamp`                                                                                | kickoff as ISO 8601 UTC; the daily job rewrites it, with `closeTime`, when the provider moves the game                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | kickoff time, day grouping, the live window                            |
...
| `sportsLiveStatus`, `sportsHomeScore`, `sportsAwayScore`, `sportsLiveMinute`, `sportsLiveUpdatedTime` | written by the resolve job: `IN_PLAY` while the provider reports a score, `FINISHED` at the final; no clock from this provider. Broadcast on `contract/{id}/sports-live` with `sportsLiveUpdatedTime`, which the page uses to drop pushes older than what it already has.                                                                                                                                                                                                                                                                                                                            | LIVE badge, score, "Just finished"                                     |
| resolution                                                                                            | the winning team's answer; for a tie, Draw where there is one, otherwise 50/50 between the teams (`gameResolution`). The side comes from the `/scores` payload's own home and away fields. Old binary markets: YES, NO, or MKT at 50%. Postponed games stay open for an admin (they show in the needs-attention list).                                                                                                                                                                                                                                                                               | Final and winner marks                                                 |
...
- **Soccer knockouts.** The MLS Cup Playoffs and the Champions League knockout phase have auto-create off: those games go to extra time and penalties, and the resolver only sees goals, so a shootout would resolve as a Draw. Decide between the 90-minute result and who advances first.
...
- **Postponements and cancellations.** A game the provider moves within the 14-day window gets its new kickoff and close time on the next daily run. Nothing resolves N/A automatically; the needs-attention list on `/admin/sports` is where the rest surface.
