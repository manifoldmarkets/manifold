...
create table if not exists tombstones (
  ...
  unique(doc_kind, parent_id, doc_id)
);
alter table tombstones enable row level security;
create index if not exists tombstones_doc_id_fs_deleted_at on tombstones (doc_kind, doc_id, fs_deleted_at desc);
  ...
  if r.write_kind = 'create' then
    ...
    */
    if exists (
      ...
      where t.doc_kind = r.doc_kind and t.doc_id = r.doc_id and t.fs_deleted_at > r.ts
    ...
    */
    ...
    insert into tombstones (doc_kind, parent_id, doc_id, fs_deleted_at) values (r.doc_kind, r.parent_id, r.doc_id, r.ts)
    on conflict (doc_kind, parent_id, doc_id) do update set fs_deleted_at = r.ts
